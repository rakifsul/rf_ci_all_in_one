...
<div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        ...
        <hr>
        <a href="<?= site_url("/admin/attachment"); ?>"><span class="badge bg-dark">all</span></a>
        <?php foreach($resultTags as $item) { ?>
            <a href="<?= site_url("/admin/attachment/?tag=".$item); ?>"><span class="bdg badge"><?= $item ?></span></a>
        <?php } ?>
        <hr>
            ...
            <tbody>
                <?php foreach($resultFiles as $item) { ?>
                    <tr>
                        ...
                        <td><?= $item["path"] ?></td>
                        <td>
                            <a class="btn btn-warning btn-smaller" href="<?= site_url("/admin/attachment/do_download/".$item["id"]); ?>">DL</a>
                            <button class="btn-edit-file btn btn-success btn-smaller" data-title="<?= $item["title"] ?>">ED</button>
                            <a class="btn btn-danger btn-smaller" href="<?= site_url("/admin/attachment/do_delete/".$item["id"]); ?>">DT</a>
                        </td>
                    </tr>    
                <?php } ?>
            </tbody>
        ...
        <nav aria-label="Page navigation example">
            ...
            <ul class="pagination">
                <li class="page-item <?php if ($prevPGN['disabled']) { ?>disabled<?php } ?>">
                    <a class="page-link" href="<?= site_url("/admin/attachment/?page="); ?><?= $prevPGN['index'] ?><?= $tag != "" ? "&tag=".$tag : "" ?><?= $sq != "" ? "&sq=".$sq : "" ?>">Previous</a>
                </li>

                <?php for($i = 0; $i < $pagination['pageCount']; ++$i){ ?>
                    <li class="page-item <?php if($pagination['page'] == $i){ ?> active <?php } ?>">
                        <a class="page-link" href="<?= site_url("/admin/attachment/?page="); ?><?= $i ?><?= $tag != "" ? "&tag=".$tag : "" ?><?= $sq != "" ? "&sq=".$sq : "" ?>">
                            <?= $i ?>
                        </a>
                    </li>
                <?php } ?>
                ...
                <li class="page-item <?php if ($nextPGN['disabled']) { ?>disabled<?php } ?>">
                    <a class="page-link" href="<?= site_url("/admin/attachment/?page="); ?><?= $nextPGN['index'] ?><?= $tag != "" ? "&tag=".$tag : "" ?><?= $sq != "" ? "&sq=".$sq : "" ?>">Next</a>
                </li>
            </ul>
            <!-- <ul class="pagination">
...
<div class="modal fade" id="modal-add" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="<?= site_url("/admin/attachment/do_upload"); ?>" method="post" onsubmit="return parseTagsInput('txa-tags', 'txa-div-tags')" enctype="multipart/form-data">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="<?= site_url("/admin/attachment/do_upload"); ?>" method="post" onsubmit="return parseTagsInput('txa-tags', 'txa-div-tags')" enctype="multipart/form-data">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="tx-title-edit">Title</label>
                        <input type="text" class="form-control" id="tx-title-edit" name="title">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
...
<script>
    ...
    $(`.btn-edit-file`).click(function (e) {
        ...
        $("#tx-title-edit").val($(this).attr("data-title"))
        authModal.show();
    });
    ...
    function search() {
        window.location = "<?= site_url("/admin/attachment/?sq="); ?>" + $("#query").val();
    }
</script>
<?= $this->endSection() ?>